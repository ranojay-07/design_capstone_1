...
def submit ():
    ...
    query = f"""
    INSERT INTO instructor (i_id, f_name, l_name, dept_name, email, salary)
    VALUES (%s, %s, %s, %s, %s, %s)
    """

    values = (faculty_id.get(), f_name.get(), l_name.get(), dept.get(), faculty_id.get() + "@xim.edu.in" , float(salary.get()))
    ...
    if not os.path.exists ( f"images/photos/{faculty_id.get()}.png" ):
        img.save ( f"images/photos/{faculty_id.get()}.png" )
